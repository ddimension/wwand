...
// wwand — EARFCN / NR-ARFCN -> 3GPP band derivation (daemon-side).
//
// Server-side mirror of luci-app-wwand/htdocs/luci-static/resources/wwand/
// bands.js (the LuCI status page / proto handler derive band the same way,
// client-side). This module fills serving.lte.band / serving.nr.band for the
// band-less transports — native MBIM on non-Qualcomm modems (Intel / MediaTek)
// where there is NO AT +QENG and NO QMI CA-info (serving_from_ca) to name the
// band. On those the EARFCN/NR-ARFCN is the ONLY band source, so without this
// the telemetry log line and ubus status carry no band at all (LuCI would still
// show it via its own JS table, but nothing else would).
//
// Keep the two tables in lock-step with bands.js — same 3GPP source, same
// ordering rules. Test: test_arfcn_bands.uc.
//
// NOTE: this derives BAND (and centre frequency) only. Channel BANDWIDTH is not
// recoverable from the ARFCN — that stays with AT +QENG / QMI LTE-CPHY CA-info
// (serving_from_ca). See modem_common.serving_from_ca.
...
// ranges overlap, since nr_band() returns the first match (e.g. n78 before n77,
// n1 before n65/n66). Covers the common FR1 bands + FR2 mmWave.
...
export function nr_arfcn_to_mhz(n)
{
...
// EARFCN -> { band: 'B<n>', mhz: <number> } or null (unknown/out-of-range).
export function lte_band(earfcn)
{
	if (earfcn == null)
		return null;
	...
	for (let b in LTE_BANDS)
		if (earfcn >= b[1] && earfcn <= b[2])
			return { band: sprintf('B%d', b[0]), mhz: b[3] + 0.1 * (earfcn - b[1]) };

	return null;
};

// NR-ARFCN -> { band: 'n<n>'|null, mhz: <number> } or null (arfcn absent). The
// band may be null (a valid centre frequency that falls in no tabled band); the
// caller keeps the mhz in that case.
export function nr_band(arfcn)
{
	if (arfcn == null)
		return null;

	let mhz = nr_arfcn_to_mhz(arfcn), band = null;

	for (let e in NR_BANDS)
		if (mhz >= e[1] && mhz <= e[2]) { band = e[0]; break; }
