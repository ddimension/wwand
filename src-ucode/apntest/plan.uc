...
function parse_checks(raw, where, errors)
{
	...
	for (let c in (type(raw) == 'array' ? raw : (raw != null ? [ raw ] : []))) {
		...
		let colon = index(s, ':');
		let name = (colon >= 0) ? substr(s, 0, colon) : s;
		let arg = (colon >= 0) ? substr(s, colon + 1) : null;
		...
		push(list, { name: name, arg: arg, raw: s });
	}
...
function parse_test(name, s, sims, errors)
{
	...
	return {
		...
		service: (s.service != null && s.service !== '') ? '' + s.service : name,
...
export function parse(raw)
{
	...
	for (let name in (raw ?? {})) {
		...
		push(tests, parse_test(name, s, sims, errors));
	}
